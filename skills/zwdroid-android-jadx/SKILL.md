...
**Sub-case — sources cached but stale jadx version:** compare `meta.json.jadx_version` with the current `jadx --version`. Mismatch is usually fine (decompiled .java doesn't depend on the jadx that produced it) but flag it to the user so they can choose to redo the work — there's no `--rebuild` flag on the manual jadx path, so "rebuild" means deleting `$WS` and re-running Phase 2 (full re-decompile), or just `build_xref_index.py --rebuild` if only the index needs refreshing.
